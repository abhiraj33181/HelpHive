          ...
          {selectedAppointment && (
            <div className="space-y-8 max-w-lg">
              ...
              <div className="flex items-start gap-4 pb-6 border-b border-gray-200">
                <div className="w-20 h-20 flex-shrink-0 rounded-2xl overflow-hidden shadow-lg ring-2 ring-white/50">
                  <img
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span className="truncate">{selectedAppointment.userData.address?.city}</span>
                  </div>
                </div>
              </div>

              {/* Service & Amount */}
              <div className="grid grid-cols-2 gap-4 p-5 bg-gradient-to-r from-indigo-50 to-blue-50 rounded-2xl border border-indigo-200/50">
                <div className="flex items-center gap-3">
                  <Waypoints className="h-5 w-5 text-indigo-600" />
                  <span className="font-semibold text-indigo-900 text-sm">{selectedAppointment.provData.service}</span>
                </div>
              ...
              <div>
                <h3 className="font-semibold text-lg mb-4 flex items-center gap-2 text-gray-900">
                  <MessageCircleMore className="h-5 w-5 text-indigo-600" />
                  Quick Actions
                </h3>
                <div className="grid grid-cols-3 gap-3">
                  <button
                    onClick={() => window.location.href = `tel:${selectedAppointment.userData.phone}`}
                    className="group p-4 bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center gap-2 h-20 active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="group p-4 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center gap-2 h-20 active:scale-95"
                  >
                    <MessageCircleMore className="h-6 w-6 group-hover:scale-110 transition-transform" />
                    <span className="text-xs font-semibold">Chat</span>
                  </button>

                  <button className="group p-4 bg-gray-100 text-gray-600 rounded-2xl shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col items-center gap-2 h-20 cursor-not-allowed opacity-70">
              ...
              <div className="p-5 bg-gray-50 rounded-2xl border border-gray-200">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="text-sm text-gray-700 space-y-1">
                    <div>{selectedAppointment.userData.address?.street}</div>
                    <div className="font-medium">{selectedAppointment.userData.address?.city}, {selectedAppointment.userData.address?.state}</div>
                    <div>{selectedAppointment.userData.address?.pincode}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-5 rounded-2xl border bg-gradient-to-br from-gray-50 to-white shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-semibold text-gray-700 text-sm">Booking Status</span>
                    <div className={`w-2.5 h-2.5 rounded-full ${selectedAppointment.isAccepted === 'Pending' || selectedAppointment.isAccepted === 'Appointment Request Cancelled'
                        ? 'bg-amber-400'
                        : 'bg-emerald-500'
                      }`} />
                  </div>
                  <span className={`block w-full text-center px-3 py-2 rounded-xl text-sm font-bold text-xs ${selectedAppointment.isAccepted === 'Pending' || selectedAppointment.isAccepted === 'Appointment Request Cancelled'
                ...
                <div className="p-5 rounded-2xl border bg-gradient-to-br from-gray-50 to-white shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className={`w-2.5 h-2.5 rounded-full ${selectedAppointment.payment ? 'bg-emerald-500' : 'bg-red-500'
                      }`} />
                  </div>
                  <span className={`block w-full text-center px-3 py-2 rounded-xl text-sm font-bold text-xs ${selectedAppointment.payment
                      ? 'bg-emerald-100 text-emerald-800 border border-emerald-200'
              ...
              {selectedAppointment.isAccepted === 'Pending' && (
                <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-200">
                  <button
                    ...
                    className="group py-4 px-6 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-2 text-sm active:scale-95"
                  >
                    <CheckCircle className="h-5 w-5 group-hover:scale-110" />
                    Accept
                  </button>
                  <button
                    onClick={() => updateAppointmentStatus(selectedAppointment._id, 'Appointment Request Cancelled')}
                    className="group py-4 px-6 bg-gradient-to-r from-red-500 to-rose-600 text-white font-bold rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-2 text-sm active:scale-95"
                  >
                    <XCircle className="h-5 w-5 group-hover:scale-110" />
                    Reject
                  </button>
                </div>
              )}
